...
public class HttpFormConnector {
	...
	static public int doPostRes(String url, Map<String, String> form, int timeout) throws IOException{
		...
		int ret = connector.getResponseCode();
		connector.disconnect();
	...
	static public int doPostRes(String url, byte[] data, String contentType, int timeout) throws IOException{
		URL urlConn = new URL(url);
		HttpURLConnection connector = (HttpURLConnection)urlConn.openConnection();
		connector.setConnectTimeout(timeout);
		connector.setReadTimeout(timeout);
